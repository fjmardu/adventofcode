...
exports.AOC_02_2020 = void 0;
const global_1 = require("./global");
class AOC_02_2020 {
    static abailar() {
        //console.log('Parte 1=' + this.checkPwds());
        console.log('Parte 2=' + this.checkPwds());
    }
    static checkPwds() {
        const pwdS = this.input;
        const arrayPwdS = global_1.global.inputToArrayStr(pwdS);
        let pwdOK = 0;
        arrayPwdS.forEach(pwd => {
            const aux = pwd.split('-');
            const min = parseInt(aux[0]);
            const aux2 = aux[1].split(' ');
            const max = parseInt(aux2[0]);
            const letra = aux2[1][0];
            const pwdCheck = aux2[2];
            //PARTE 1-----------------------------------------------------------------
            /* const result:string[]=pwdCheck.split(letra)
            if(result!=null && result.length-1>=min && result.length-1<=max){
                pwdOK++
            }  */
            //PARTE 1-----------------------------------------------------------------
            //PARTE 2-----------------------------------------------------------------
            if ((pwdCheck[min - 1] == letra && pwdCheck[max - 1] != letra) || (pwdCheck[min - 1] != letra && pwdCheck[max - 1] == letra)) {
                pwdOK++;
            }
            //PARTE 2-----------------------------------------------------------------
        });
...
exports.AOC_02_2020 = AOC_02_2020;
/*     public static checkPwds():number{
        let pwdOK:number=0
        this.input.forEach(pwd=>{
            const aux:string[]=pwd.split('-')
            const min:number=parseInt(aux[0])
            const aux2:string[]=aux[1].split(' ')
            const max:number = parseInt(aux2[0])
            
            const letra:string=aux2[1][0]
            
            const pwdCheck:string=aux2[2]
            
            //ahora creo un array de string por cada letra a buscar y la longitud del array será el número de veces que está la letra en la pwd
            const result:string[]=pwdCheck.split(letra)
