...
exports.AOC_06_2020 = void 0;
const global_1 = require("./global");
class AOC_06_2020 {
    static abailar() {
        //console.log('Result first part test='+this.onePart(this.inputTest));         
        //console.log('Result first part='+this.onePart(this.input));         
        console.log('Result second part test=' + this.secondPart(this.inputTest));
        console.log('Result second part test=' + this.secondPart(this.input));
    }
    static secondPart(input) {
        let salida = 0;
        const mInput = global_1.global.inputToStrArrayByBlankLine(input);
        mInput.forEach(data => {
            const respond = data.split(" ");
            let result = respond[0];
            for (let i = 0; i < respond.length; i++) {
                if (result !== respond[i]) {
                    let newResult = "";
                    for (let j = 0; j < result.length; j++) {
                        const index = respond[i].search(result[j]);
                        //console.log("Está " + result[j] + " en " + respond[i] + '=' + index);
                        if (index !== -1)
                            newResult += result[j];
                    }
                    result = newResult;
            ...
            salida += result.length;
        });
        return salida;
    }
    static onePart(input) {
        const mInput = global_1.global.inputToStrArrayByBlankLine(input);
        let salida = 0;
        mInput.forEach(form => {
            let result = "";
            const form_ = form.replace(/ /g, "");
            for (let i = 0; i < form_.length; i++) {
                const index = result.search(form_[i]);
                if (index === -1)
                    result = result + form_[i];
            }
            salida += result.length;
        });
        return salida;
...
exports.AOC_06_2020 = AOC_06_2020;
AOC_06_2020.inputTest = `abc
...
b`;
AOC_06_2020.input = `donpevkjhymzl
ezyopckdlnvmj
