...
class AOC_07_2020 {
    ...
    static secondPart(input) {
        const maletas = global_1.global.inputToArrayStr(input);
        //limpio el array para dejarlo guay 
        const cleanMaletas = maletas.map(maleta => {
            let regex = / bags/g;
        ...
        let resultado = 0;
        const calcula = (maleta, factor = 1) => {
            const salida = this.maletasContained(cleanMaletas, maleta);
            if (salida.search(',') !== -1) {
                const array = salida.split(', ');
    ...
    static maletasContained(maletas, maleta) {
        const maletasArray = maletas.map(maleta => {
    ...
    static firstPart(input) {
        const maletas = global_1.global.inputToArrayStr(input);
        //qué maletas contienen una shiny gold bag
        let maletasResult = this.maletasContain(maletas, 'shiny gold');
        //ahora busco qué maletas pueden contener a mi resultado       
        let newMaletas = [...maletasResult];
        while (newMaletas.length !== 0) {
            let aux = [];
            newMaletas.forEach(maleta => {
                const nResult = this.maletasContain(maletas, maleta);
                if (nResult.length !== 0) {
...
exports.AOC_07_2020 = AOC_07_2020;
AOC_07_2020.inputTest = `light red bags contain 1 bright white bag, 2 muted yellow bags.
...
AOC_07_2020.inputTest2 = `shiny gold bags contain 2 dark red bags.
...
AOC_07_2020.input = `light orange bags contain 1 dark maroon bag, 3 dim maroon bags, 5 striped green bags, 2 pale aqua bags.
