...
export class AOC_06_2020{
            ...
            for(let i=0;i<respond.length;i++){
                if(result!==respond[i]){
                    ...
                    for(let j=0;j<result.length;j++){                    
                        const index:number= respond[i].search(result[j])
                        //console.log("Está " + result[j] + " en " + respond[i] + '=' + index);
                        
                        if (index!==-1)newResult+=result[j]
                    }
                    result=newResult
                }
            }
    ...
    private static onePart(input:string):number{
        ...
        mInput.forEach(form=>{
            let result:string=""
            const form_:string=form.replace(/ /g, "")
            for(let i=0; i<form_.length;i++){
                const index:number= result.search(form_[i])                
                if(index===-1)result=result+form_[i]
            }            
            salida+=result.length            
        })        
        return salida
